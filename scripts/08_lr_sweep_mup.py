...
08_lr_sweep_mup.py
------------------
Learning rate sweep on the Tiny µP model. Identical protocol to 06_lr_sweep.py
but uses MupTransformerLM + MuAdamW + set_base_shapes.

Saves: outputs/logs/lr_sweep_mup.json
       outputs/plots/lr_sweep_mup.png

Usage:
    python scripts/08_lr_sweep_mup.py [--max_steps 2000]
"""
...
def plot_lr_sweep_comparison(sp_path: Path, mup_results: list[dict], save_path: Path) -> None:
    ...
    try:
        ...
        if sp_path.exists():
            ...
            best_sp = sp["best_val_loss"]
            ax.plot(lrs_sp, vls_sp, "o--", color="steelblue", label="SP", linewidth=1.5)
            ax.scatter([sp["best_lr"]], [best_sp],
                       color="steelblue", s=150, marker="*", zorder=6,
                       label=f"SP best: lr={sp['best_lr']:.1e}, val={best_sp:.4f}")

        ax.set_xscale("log")
        ax.set_xlabel("Learning rate (log scale)")
        ax.set_ylabel("Final validation loss")
        ax.set_title("LR Sweep Comparison — Tiny model: SP vs µP")
        ax.legend()
        ...
        plt.close()
        print(f"LR sweep comparison plot saved to {save_path}")
...
def main():
    ...
    print(f"µP LR sweep: {len(lr_values)} values × {max_steps} steps each")
    ...
    print(f"\nBest µP LR: {best['lr']:.1e}  →  val_loss={best['val_loss']:.4f}")
    ...
    out_path = log_dir / "lr_sweep_mup.json"
    ...
    plot_lr_sweep_comparison(
        ...
        mup_results=results,
        save_path=plots_dir / "lr_sweep_comparison.png",
    )
