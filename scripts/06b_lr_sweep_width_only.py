"""
06b_lr_sweep_width_only.py
--------------------------
SP learning rate sweep on the w_xs model (width-only family, d_model=128).

Identical protocol to 06_lr_sweep.py but targets the width_only family so
the result is directly comparable to the µP sweep in 08b_lr_sweep_mup_width_only.py.
...
Usage:
    python scripts/06b_lr_sweep_width_only.py [--max_steps 2000]
"""
...
def run_one_lr(
    ...
    model = build_model("w_xs", config_family="width_only").to(device)
...
def plot_lr_sweep(results: list[dict], save_path: Path) -> None:
    try:
        ...
        ax.set_title("LR Sweep — w_xs (SP, width-only family)\n(red = diverged)")
        ax.legend()
...
def main():
    ...
    print(f"SP LR sweep (width_only / w_xs): {len(lr_values)} values × {max_steps} steps each")
    ...
    print(f"\nBest SP LR (width_only): {best['lr']:.1e}  →  val_loss={best['val_loss']:.4f}")
    ...
    out_path = log_dir / "lr_sweep_width_only_sp.json"
    with open(out_path, "w") as f:
        json.dump(sweep_result, f, indent=2)
    print(f"Results saved to {out_path}")

    plot_lr_sweep(results, plots_dir / "lr_sweep_width_only_sp.png")
